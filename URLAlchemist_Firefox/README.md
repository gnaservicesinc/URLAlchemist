...
- Version 2.7.2 uses schema 9 workspace/action-pack artifacts, preserves V1 `.urlpack` conversion, keeps Custom Block category and boundary metadata synchronized, stores large resources in IndexedDB by SHA-256, and strips local-only install metadata from exported `.actionpack` files.

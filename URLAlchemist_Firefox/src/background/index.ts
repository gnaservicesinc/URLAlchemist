...
async function readClipboardFromFirefox(): Promise<string> {
  ...
  if (new TextEncoder().encode(text).byteLength > CLIPBOARD_MAX_TEXT_BYTES) {
    throw new Error(`Clipboard text exceeds the ${CLIPBOARD_MAX_TEXT_BYTES / 1024 / 1024}MB size limit`);
  }

  return text;
}

async function writeClipboardFromFirefox(text: string): Promise<void> {
  const permissionGranted = await chrome.permissions.contains({
...
async function writeClipboardBinaryFromFirefox(mimeType: string, dataBase64: string): Promise<void> {
  const permissionGranted = await chrome.permissions.contains({
    permissions: ['clipboardWrite'],
  });
  ...
  if (!navigator.clipboard?.write || typeof ClipboardItem === 'undefined') {
    throw new Error('Binary clipboard writes are unavailable in this Firefox context');
  }
...
const baseRuntime: GraphRuntime = {
  ...
  readClipboard: readClipboardFromFirefox,
...
function createRunRuntime(context: RuntimeSourceContext = {}, settings?: GlobalSettings, packId?: string, inputUrl?: string, packName = '', loggingEnabled = true): GraphRuntime {
  return {
    ...baseRuntime,
    regex: createPageRegexExecutor(settings ? effectiveRegexTimeoutMs(settings) : undefined),
    ...
    readSource: async (source) => {
      if (source === 'clipboard') {
        return { type: 'string', value: await readClipboardFromFirefox() };
      }
    ...
    writeDestination: async (destination, value) => {
      if (destination === 'clipboard') {
        await writeClipboardFromFirefox(typeof value.value === 'string' ? value.value : JSON.stringify(value.value));
      } else if (destination === 'clipboardBinary') {
        const { mimeType, dataBase64 } = getBinaryClipboardPayload(value);
        await writeClipboardBinaryFromFirefox(mimeType, dataBase64);
      }
    },
...
async function runConditionalPack(packId: string): Promise<void> {
  ...
  const runtime = createRunRuntime({ tabId: tab.id, pageTitle: tab.title }, state.settings, pack.manifest.id, tab.url, pack.manifest.name, pack.install?.loggingEnabled !== false);
